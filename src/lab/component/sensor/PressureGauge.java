...
public class PressureGauge extends MeasurableComponent{
	...
	public PressureGauge(int width, int height, String title, int sigfigs) {
		super(width, height);
		setValue(0);
	...
	public void draw(int x, int y, int width, int height, Graphics g) {
		...
		gauge.setText(SigFig.sigfigalize(this.getValue(),sigfigs));
		...
		gauge.getJComponent().setFont(gauge.getJComponent().getFont().deriveFont(2*sigfigs*(float)(Math.sqrt(width*width+height*height)/Math.sqrt(gauge.getWidth()*gauge.getWidth()+gauge.getHeight()*gauge.getWidth()))));
