...
public class Graph extends LabComponent implements MouseListener, MouseMotionListener {
	...
	public Collection<GraphDataSet> getDataSets() {
		return data.values();
	}
	...
	public void draw(int x, int y, int width, int height, Graphics g) {
		...
		for (GraphDataSet dataSet : data.values()) {
			...
			if (dataSet.isShowName() && dataSet.getPoints().size() > 0) {
				g.drawString(dataSet.getName(), (int) (dx + metrics.stringWidth(dataSet.getName()) / 2) + dataSet.getNameOffsetX(), (int) dy + 2 + dataSet.getNameOffsetY());
			}
